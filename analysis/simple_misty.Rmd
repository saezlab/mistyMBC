...
Define standard view composition and run MISTy on all h5ad files that contain a modality name in their filename passed to the renderer via a `modality` parameter in the YAML header (default: "codex").
...
(list.files("data", ".h5ad", recursive = TRUE, full.names = TRUE) %>%
  keep(~ str_detect(.x, params$modality))) %>%
